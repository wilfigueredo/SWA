...
export class LogoutComponent implements OnInit {
    ...
    switch (action.path) {
      case LogoutActions.Logout:
        if (!!window.history.state.local) {
          ...
          this.message.next('O logout não foi iniciado a partir da página.');
        }
      ...
      case LogoutActions.LoggedOut:
        this.message.next('Você efetuou o logout com sucesso!');
        break;
      default:
        throw new Error(`Ação Invalida '${action}'`);
    }
  }
    ...
    if (isauthenticated) {
      ...
      switch (result.status) {
        ...
        case AuthenticationResultStatus.Fail:
          ...
          break;
        default:
          throw new Error('Status da autenticação invalido.');
      }
    } else {
      this.message.next('Você efetuou o logout com sucesso!');
    }
  ...
  private getReturnUrl(state?: INavigationState): string {
    ...
    if (fromQuery &&
      ...
      throw new Error('URL invalida.O retorno precisa ter a mesma origem da página atual.');
    }
